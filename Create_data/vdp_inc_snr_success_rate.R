...
prediction_model_names <- paste(vdp_col_names, "_", sep="")
...
## read files -----------------------------------------
x_dot_lasso_ci <- read.csv('pred_model_csv/SNR/vdp_inc_snr_xdot_lasso_ci_df.csv')[,-1]
x_dot_lasso_pe <- read.csv('pred_model_csv/SNR/vdp_inc_snr_xdot_lasso_pe_df.csv')[,-1]
y_dot_lasso_ci <- read.csv('pred_model_csv/SNR/vdp_inc_snr_ydot_lasso_ci_df.csv')[,-1]
y_dot_lasso_pe <- read.csv('pred_model_csv/SNR/vdp_inc_snr_ydot_lasso_pe_df.csv')[,-1]

x_dot_alasso_ci <- read.csv('pred_model_csv/SNR/vdp_inc_snr_xdot_alasso_ci_df.csv')[,-1]
x_dot_alasso_pe <- read.csv('pred_model_csv/SNR/vdp_inc_snr_xdot_alasso_pe_df.csv')[,-1]
y_dot_alasso_ci <- read.csv('pred_model_csv/SNR/vdp_inc_snr_ydot_alasso_ci_df.csv')[,-1]
y_dot_alasso_pe <- read.csv('pred_model_csv/SNR/vdp_inc_snr_ydot_alasso_pe_df.csv')[,-1]

x_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/vdp_inc_snr_xdot_pysindy_pe_df.csv')[,-1]
y_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/vdp_inc_snr_ydot_pysindy_pe_df.csv')[,-1]
...
for (i in seq_len(nrow(ydot_lasso_prediction_model))) {
  if (
    ...
    ydot_lasso_prediction_model[i,]$xxy != 0 &&
    rowSums(ydot_lasso_prediction_model[i,][, !names(ydot_lasso_prediction_model) %in% c("x", "y", "xxy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(ydot_alasso_prediction_model))) {
  if (
    ...
    ydot_alasso_prediction_model[i,]$xxy != 0 &&
    rowSums(ydot_alasso_prediction_model[i,][, !names(ydot_alasso_prediction_model) %in% c("x", "y", "xxy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(ydot_pysindy_prediction_model))) {
  if (
    ...
    ydot_pysindy_prediction_model[i,]$xxy != 0 &&
    rowSums(ydot_pysindy_prediction_model[i,][, !names(ydot_pysindy_prediction_model) %in% c("x", "y", "xxy")]
            != 0) == 0) {
...
write.csv(xdot_lasso_prediction_model,
          "stacked_bar_csv/SNR/vdp_inc_n_xdot_lasso_pred_models_new_sg.csv")
write.csv(ydot_lasso_prediction_model,
          "stacked_bar_csv/SNR/vdp_inc_n_ydot_lasso_pred_models_new_sg.csv")

write.csv(xdot_alasso_prediction_model,
          "stacked_bar_csv/SNR/vdp_inc_n_xdot_alasso_pred_models_new_sg.csv")
write.csv(ydot_alasso_prediction_model,
          "stacked_bar_csv/SNR/vdp_inc_n_ydot_alasso_pred_models_new_sg.csv")

write.csv(xdot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/vdp_inc_n_xdot_sindy_pred_models_new_sg.csv")
write.csv(ydot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/vdp_inc_n_ydot_sindy_pred_models_new_sg.csv")
...
save(eta, total_correct_increasing_snr_df, file = "success_rate_RData/vdp_inc_snr_success_rate_new_sg.RData")
