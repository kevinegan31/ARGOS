...
file_wd2 <- paste(file_wd, "Data/Lorenz/", sep = "") # 
setwd(file_wd2)
### Read function
...
lorenz_col_names <- get_can_names(lorenz_system(100,c(2,5,10),0.001))
prediction_model_names <- paste(lorenz_col_names, "_", sep="")
...
## read files -----------------------------------------
x_dot_lasso_ci <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_xdot_lasso_ci_df.csv')[,-1]
x_dot_lasso_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_xdot_lasso_pe_df.csv')[,-1]
y_dot_lasso_ci <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_ydot_lasso_ci_df.csv')[,-1]
y_dot_lasso_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_ydot_lasso_pe_df.csv')[,-1]
z_dot_lasso_ci <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_zdot_lasso_ci_df.csv')[,-1]
z_dot_lasso_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_zdot_lasso_pe_df.csv')[,-1]

x_dot_alasso_ci <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_xdot_alasso_ci_df.csv')[,-1]
x_dot_alasso_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_xdot_alasso_pe_df.csv')[,-1]
y_dot_alasso_ci <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_ydot_alasso_ci_df.csv')[,-1]
y_dot_alasso_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_ydot_alasso_pe_df.csv')[,-1]
z_dot_alasso_ci <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_zdot_alasso_ci_df.csv')[,-1]
z_dot_alasso_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_zdot_alasso_pe_df.csv')[,-1]

x_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_xdot_pysindy_pe_df.csv')[,-1]
y_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_ydot_pysindy_pe_df.csv')[,-1]
z_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/lorenz_inc_snr_zdot_pysindy_pe_df.csv')[,-1]
...
for (i in seq_len(nrow(ydot_lasso_prediction_model))) {
  if (
    ...
    ydot_lasso_prediction_model[i,]$xz != 0 &&
    rowSums(ydot_lasso_prediction_model[i,][, !names(ydot_lasso_prediction_model) %in% c("x", "y", "xz")]
            != 0) == 0) {
...
for (i in seq_len(nrow(zdot_lasso_prediction_model))) {
  if (
    ...
    zdot_lasso_prediction_model[i,]$xy != 0 &&
    rowSums(zdot_lasso_prediction_model[i,][, !names(zdot_lasso_prediction_model) %in% c("z", "xy")]
            != 0) == 0) {
...
# Rename columns
colnames(zdot_lasso_eta_df) <- paste("eta_", eta)
...
for (i in seq_len(nrow(ydot_alasso_prediction_model))) {
  if (
    ...
    ydot_alasso_prediction_model[i,]$xz != 0 &&
    rowSums(ydot_alasso_prediction_model[i,][, !names(ydot_alasso_prediction_model) %in% c("x", "y", "xz")]
            != 0) == 0) {
...
for (i in seq_len(nrow(zdot_alasso_prediction_model))) {
  if (
    ...
    zdot_alasso_prediction_model[i,]$xy != 0 &&
    rowSums(zdot_alasso_prediction_model[i,][, !names(zdot_alasso_prediction_model) %in% c("z", "xy")]
            != 0) == 0) {
...
# Rename columns
colnames(zdot_alasso_eta_df) <- paste("eta_", eta)
###
...
# ggplot(alasso_total_correct_eta_df, aes(eta_0.24)) +
#   geom_histogram(fill = "#0073C2FF", binwidth = 1)
# ggplot(alasso_total_correct_eta_df,
#        aes(x = eta_0.1)) +
#   geom_bar()
# 
# alasso_eta_gather_df <- gather(alasso_total_correct_eta_df, "Eta", "Value",
#                                1:ncol(alasso_total_correct_eta_df))
# alasso_total_correct_eta_df <- alasso_total_correct_eta_df[, c(1:31)]
# colnames(alasso_total_correct_eta_df) <- eta[c(1:31)]
...
# 
alasso_eta_gather_df$eta <-
  factor(alasso_eta_gather_df$eta, levels = unique(alasso_eta_gather_df$eta))

### sindy ----------------------------------
### xdot
### Count number of times model is correct per 100 rows
xdot_pysindy_prediction_model <- x_dot_pysindy_pe
...
### ydot
ydot_pysindy_prediction_model <- y_dot_pysindy_pe
...
for (i in seq_len(nrow(ydot_pysindy_prediction_model))) {
  if (
    ...
    ydot_pysindy_prediction_model[i,]$xz != 0 &&
    rowSums(ydot_pysindy_prediction_model[i,][, !names(ydot_pysindy_prediction_model) %in% c("x", "y", "xz")]
            != 0) == 0) {
...
### zdot
zdot_pysindy_prediction_model <- z_dot_pysindy_pe
...
for (i in seq_len(nrow(zdot_pysindy_prediction_model))) {
  if (
    ...
    zdot_pysindy_prediction_model[i,]$xy != 0 &&
    rowSums(zdot_pysindy_prediction_model[i,][, !names(zdot_pysindy_prediction_model) %in% c("z", "xy")]
            != 0) == 0) {
...
write.csv(xdot_lasso_prediction_model,
          "stacked_bar_csv/SNR/lorenz_inc_snr_xdot_lasso_pred_models_new_sg.csv")
write.csv(ydot_lasso_prediction_model,
          "stacked_bar_csv/SNR/lorenz_inc_snr_ydot_lasso_pred_models_new_sg.csv")
write.csv(zdot_lasso_prediction_model,
          "stacked_bar_csv/SNR/lorenz_inc_snr_zdot_lasso_pred_models_new_sg.csv")

write.csv(xdot_alasso_prediction_model,
          "stacked_bar_csv/SNR/lorenz_inc_snr_xdot_alasso_pred_models_new_sg.csv")
write.csv(ydot_alasso_prediction_model,
          "stacked_bar_csv/SNR/lorenz_inc_snr_ydot_alasso_pred_models_new_sg.csv")
write.csv(zdot_alasso_prediction_model,
          "stacked_bar_csv/SNR/lorenz_inc_snr_zdot_alasso_pred_models_new_sg.csv")

write.csv(xdot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/orenz_inc_snr_xdot_sindy_pred_models_new_sg.csv")
write.csv(ydot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/orenz_inc_snr_ydot_sindy_pred_models_new_sg.csv")
write.csv(zdot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/orenz_inc_snr_zdot_sindy_pred_models_new_sg.csv")
...
save(eta, total_correct_increasing_snr_df, file = "success_rate_RData/lorenz_inc_snr_success_rate_new_sg.RData")
