...
## read files -----------------------------------------
x_dot_lasso_ci <- read.csv('pred_model_csv/SNR/LV_inc_snr_xdot_lasso_ci_df.csv')[,-1]
x_dot_lasso_pe <- read.csv('pred_model_csv/SNR/LV_inc_snr_xdot_lasso_pe_df.csv')[,-1]
y_dot_lasso_ci <- read.csv('pred_model_csv/SNR/LV_inc_snr_ydot_lasso_ci_df.csv')[,-1]
y_dot_lasso_pe <- read.csv('pred_model_csv/SNR/LV_inc_snr_ydot_lasso_pe_df.csv')[,-1]

x_dot_alasso_ci <- read.csv('pred_model_csv/SNR/LV_inc_snr_xdot_alasso_ci_df.csv')[,-1]
x_dot_alasso_pe <- read.csv('pred_model_csv/SNR/LV_inc_snr_xdot_alasso_pe_df.csv')[,-1]
y_dot_alasso_ci <- read.csv('pred_model_csv/SNR/LV_inc_snr_ydot_alasso_ci_df.csv')[,-1]
y_dot_alasso_pe <- read.csv('pred_model_csv/SNR/LV_inc_snr_ydot_alasso_pe_df.csv')[,-1]

x_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/LV_inc_snr_xdot_pysindy_pe_df.csv')[,-1]
y_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/LV_inc_snr_ydot_pysindy_pe_df.csv')[,-1]
...
### xdot
### Count number of times model is correct per 100 rows
xdot_pysindy_aic_prediction_model <- data.frame(x_dot_pysindy_pe)
colnames(xdot_pysindy_aic_prediction_model) <-
  sub("\\_.*", "", colnames(xdot_pysindy_aic_prediction_model))
xdot_pysindy_aic_correct_rows <- rep(NA, nrow(xdot_pysindy_aic_prediction_model))
for (i in seq_len(nrow(xdot_pysindy_aic_prediction_model))) {
  if (
    xdot_pysindy_aic_prediction_model[i,]$x != 0 &&
    xdot_pysindy_aic_prediction_model[i,]$xy != 0 &&
    rowSums(xdot_pysindy_aic_prediction_model[i,][, !names(xdot_pysindy_aic_prediction_model) %in% c("x", "xy")]
            != 0) == 0) {
    xdot_pysindy_aic_correct_rows[i] <- 1
  } else {
    xdot_pysindy_aic_correct_rows[i] <- 0
  }
}
### df
xdot_pysindy_aic_eta_df <-
  as.data.frame(matrix(xdot_pysindy_aic_correct_rows, nrow = 100))
# Rename columns
colnames(xdot_pysindy_aic_eta_df) <- paste("eta_", eta)
### ydot
ydot_pysindy_aic_prediction_model <- data.frame(y_dot_pysindy_pe)
colnames(ydot_pysindy_aic_prediction_model) <-
  sub("\\_.*", "", colnames(ydot_pysindy_aic_prediction_model))
ydot_pysindy_aic_correct_rows <- rep(NA, nrow(ydot_pysindy_aic_prediction_model))
for (i in seq_len(nrow(ydot_pysindy_aic_prediction_model))) {
  if (
    ydot_pysindy_aic_prediction_model[i,]$y != 0 &&
    ydot_pysindy_aic_prediction_model[i,]$xy != 0 &&
    rowSums(ydot_pysindy_aic_prediction_model[i,][, !names(ydot_pysindy_aic_prediction_model) %in% c("y", "xy")]
            != 0) == 0) {
    ydot_pysindy_aic_correct_rows[i] <- 1
  } else {
    ydot_pysindy_aic_correct_rows[i] <- 0
  }
}
### df
ydot_pysindy_aic_eta_df <-
  as.data.frame(matrix(ydot_pysindy_aic_correct_rows, nrow = 100))
# Rename columns
colnames(ydot_pysindy_aic_eta_df) <- paste("eta_", eta)
###
pysindy_aic_total_correct_eta <- rep(NA, nrow(xdot_pysindy_aic_eta_df))
pysindy_total_correct_eta_df <- as.data.frame(matrix(NA, ncol = 11, nrow = 100))
for (i in seq_len(ncol(xdot_pysindy_aic_eta_df))) {
  new_df <- cbind(xdot_pysindy_aic_eta_df[, i],
                  ydot_pysindy_aic_eta_df[, i])
  for (j in seq_len(nrow(new_df))) {
    pysindy_aic_identification <- rep(NA, nrow(xdot_pysindy_aic_eta_df))
    if (any(new_df[j, ] == 0)) {
      pysindy_aic_total_correct_eta[j] <- 0
    } else {
      pysindy_aic_total_correct_eta[j] <- 1
    }
  }
  pysindy_total_correct_eta_df[, i] <- pysindy_aic_total_correct_eta
}
...
xdot_pysindy_prediction_models_ordered <-
  bind_rows(xdot_pysindy_prediction_models_list, .id = "column_label")
...
write.csv(xdot_lasso_prediction_model,
          "stacked_bar_csv/SNR/LV_inc_snr_xdot_lasso_pred_models_new_sg.csv")
write.csv(ydot_lasso_prediction_model,
          "stacked_bar_csv/SNR/LV_inc_snr_ydot_lasso_pred_models_new_sg.csv")

write.csv(xdot_alasso_prediction_model,
          "stacked_bar_csv/SNR/LV_inc_snr_xdot_alasso_pred_models_new_sg.csv")
write.csv(ydot_alasso_prediction_model,
          "stacked_bar_csv/SNR/LV_inc_snr_ydot_alasso_pred_models_new_sg.csv")

write.csv(xdot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/LV_inc_snr_xdot_sindy_pred_models_new_sg.csv")
write.csv(ydot_pysindy_prediction_model,
          "stacked_bar_csv/SNR/LV_inc_snr_ydot_sindy_pred_models_new_sg.csv")
...
save(eta, total_correct_increasing_snr_df, file = "success_rate_RData/LV_inc_snr_success_rate_new_sg.RData")
