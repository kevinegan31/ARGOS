...
cubic2d_col_names <- get_can_names(cubic2d_system(100,c(2,0),0.01))
prediction_model_names <- paste(cubic2d_col_names, "_", sep="")
...
## read files -----------------------------------------
x_dot_lasso_ci <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_xdot_lasso_ci_df.csv')[,-1]
x_dot_lasso_pe <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_xdot_lasso_pe_df.csv')[,-1]
y_dot_lasso_ci <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_ydot_lasso_ci_df.csv')[,-1]
y_dot_lasso_pe <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_ydot_lasso_pe_df.csv')[,-1]

x_dot_alasso_ci <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_xdot_alasso_ci_df.csv')[,-1]
x_dot_alasso_pe <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_xdot_alasso_pe_df.csv')[,-1]
y_dot_alasso_ci <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_ydot_alasso_ci_df.csv')[,-1]
y_dot_alasso_pe <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_ydot_alasso_pe_df.csv')[,-1]

x_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_xdot_pysindy_pe_df.csv')[,-1]
y_dot_pysindy_pe <- read.csv('pred_model_csv/SNR/cubic2d_inc_snr_ydot_pysindy_pe_df.csv')[,-1]
...
for (i in seq_len(nrow(xdot_lasso_prediction_model))) {
  if (
    xdot_lasso_prediction_model[i,]$xxx != 0 &&
    xdot_lasso_prediction_model[i,]$yyy != 0 &&
    rowSums(xdot_lasso_prediction_model[i,][, !names(xdot_lasso_prediction_model) %in% c("xxx", "yyy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(ydot_lasso_prediction_model))) {
  if (
    ydot_lasso_prediction_model[i,]$xxx != 0 &&
    ydot_lasso_prediction_model[i,]$yyy != 0 &&
    rowSums(ydot_lasso_prediction_model[i,][, !names(ydot_lasso_prediction_model) %in% c("xxx", "yyy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(xdot_alasso_prediction_model))) {
  if (
    xdot_alasso_prediction_model[i,]$xxx != 0 &&
    xdot_alasso_prediction_model[i,]$yyy != 0 &&
    rowSums(xdot_alasso_prediction_model[i,][, !names(xdot_alasso_prediction_model) %in% c("xxx", "yyy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(ydot_alasso_prediction_model))) {
  if (
    ydot_alasso_prediction_model[i,]$xxx != 0 &&
    ydot_alasso_prediction_model[i,]$yyy != 0 &&
    rowSums(ydot_alasso_prediction_model[i,][, !names(ydot_alasso_prediction_model) %in% c("xxx", "yyy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(xdot_pysindy_prediction_model))) {
  if (
    xdot_pysindy_prediction_model[i,]$xxx != 0 &&
    xdot_pysindy_prediction_model[i,]$yyy != 0 &&
    rowSums(xdot_pysindy_prediction_model[i,][, !names(xdot_pysindy_prediction_model) %in% c("xxx", "yyy")]
            != 0) == 0) {
...
for (i in seq_len(nrow(ydot_pysindy_prediction_model))) {
  if (
    ydot_pysindy_prediction_model[i,]$xxx != 0 &&
    ydot_pysindy_prediction_model[i,]$yyy != 0 &&
    rowSums(ydot_pysindy_prediction_model[i,][, !names(ydot_pysindy_prediction_model) %in% c("xxx", "yyy")]
            != 0) == 0) {
...
write.csv(xdot_lasso_prediction_model,
          "stacked_bar_csv/SNR/cubic2d_inc_snr_xdot_lasso_pred_models_new_sg.csv")
write.csv(ydot_lasso_prediction_model,
          "stacked_bar_csv/SNR/cubic2d_inc_snr_ydot_lasso_pred_models_new_sg.csv")

## alasso 
write.csv(xdot_alasso_prediction_model,
          "stacked_bar_csv/SNR/cubic2d_inc_snr_xdot_alasso_pred_models_new_sg.csv")
write.csv(ydot_alasso_prediction_model,
          "stacked_bar_csv/SNR/cubic2d_inc_snr_ydot_alasso_pred_models_new_sg.csv")
...
save(eta, total_correct_increasing_snr_df, file = "success_rate_RData/cubic2d_inc_snr_success_rate_new_sg.RData")
