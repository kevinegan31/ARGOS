...
init_sequence <- list()
for (i in 1:num_init) {
  init_sequence[[i]] <- 10 ^ (runif(num_state_var, min=-1, max=3)) # x1, x2 initial condition bounds
}
...
linear2d_sg_combinations_x1 <- lapply(X = xt_1,
                                    FUN = sg_optimal_combination,
                                    dt = dt)
linear2d_porder_wl_combos_x1 <- sapply(linear2d_sg_combinations_x1,
                                     function(x)
                                       x[2])
linear2d_sg_combinations_x2 <- lapply(X = xt_2,
                                    FUN = sg_optimal_combination,
                                    dt = dt)
linear2d_porder_wl_combos_x2 <- sapply(linear2d_sg_combinations_x2,
                                     function(x)
                                       x[2])
###############################################################################
...
for (i in seq_along(x_t_train)) {
  xdot_argos[[i]] <- boot_lasso(x_t = x_t_train[[i]],
                                ...
                                sg_combinations_x1 = linear2d_porder_wl_combos_x1[[i]],
                                sg_combinations_x2 = linear2d_porder_wl_combos_x2[[i]],
...
if (lasso_method == "lasso") {
  write.csv(
    xdot_pe_df,
    sprintf(
      "~/xdot_linear2d_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    xdot_ci_normal_df,
    sprintf(
      "~/xdot_linear2d_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    xdot_pe_df,
    sprintf(
      "~/xdot_linear2d_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
  ...
  write.csv(
    xdot_ci_normal_df,
    sprintf(
      "~/xdot_linear2d_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
...
if (lasso_method == "lasso") {
  write.csv(
    ydot_pe_df,
    sprintf(
      "~/ydot_linear2d_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    ydot_ci_normal_df,
    sprintf(
      "~/ydot_linear2d_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    ydot_pe_df,
    sprintf(
      "~/ydot_linear2d_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
  ...
  write.csv(
    ydot_ci_normal_df,
    sprintf(
      "~/ydot_linear2d_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
