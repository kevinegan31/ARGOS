...
for (i in seq_along(x_t_train)) {
  xdot_isr[[i]] <- boot_lasso(x_t = x_t_train[[i]],
                              ...
                              sg_combinations_x1 = lorenz_porder_wl_combos_x1[[i]],
                              sg_combinations_x2 = lorenz_porder_wl_combos_x2[[i]],
                              sg_combinations_x3 = lorenz_porder_wl_combos_x3[[i]],
...
if (lasso_method == "lasso") {
  write.csv(xdot_pe_df,
            sprintf("~/xdot_lorenz_increasing_snr_%s_N%s_B%s_snr_%s_%s_SEED_%s_pe_df.csv",
                    lasso_method, n_obs, num_samples, start, end, seed))
  write.csv(xdot_ci_df,
            sprintf("~/xdot_lorenz_increasing_snr_%s_N%s_B%s_snr_%s_%s_SEED_%s_ci_df.csv",
                    lasso_method, n_obs, num_samples, start, end, seed))
} else {
  write.csv(xdot_pe_df,
            sprintf("~/xdot_lorenz_increasing_snr_%s_%s_N%s_B%s_snr_%s_%s_SEED_%s_pe_df.csv",
                    lasso_method, alasso_weights_method, n_obs, num_samples, start, end, seed))
  write.csv(xdot_ci_df,
            sprintf("~/xdot_lorenz_increasing_snr_%s_%s_N%s_B%s_snr_%s_%s_SEED_%s_ci_df.csv",
                    lasso_method, alasso_weights_method, n_obs, num_samples, start, end, seed))
}
...
if (lasso_method == "lasso") {
  write.csv(ydot_pe_df,
            sprintf("~/ydot_lorenz_increasing_snr_%s_N%s_B%s_snr_%s_%s_SEED_%s_pe_df.csv",
                    lasso_method, n_obs, num_samples, start, end, seed))
  write.csv(ydot_ci_df,
            sprintf("~/ydot_lorenz_increasing_snr_%s_N%s_B%s_snr_%s_%s_SEED_%s_ci_df.csv",
                    lasso_method, n_obs, num_samples, start, end, seed))
} else {
  write.csv(ydot_pe_df,
            sprintf("~/ydot_lorenz_increasing_snr_%s_%s_N%s_B%s_snr_%s_%s_SEED_%s_pe_df.csv",
                    lasso_method, alasso_weights_method, n_obs, num_samples, start, end, seed))
  write.csv(ydot_ci_df,
            sprintf("~/ydot_lorenz_increasing_snr_%s_%s_N%s_B%s_snr_%s_%s_SEED_%s_ci_df.csv",
                    lasso_method, alasso_weights_method, n_obs, num_samples, start, end, seed))
}
...
if (lasso_method == "lasso") {
  write.csv(zdot_pe_df,
            sprintf("~/zdot_lorenz_increasing_snr_%s_N%s_B%s_snr_%s_%s_SEED_%s_pe_df.csv",
                    lasso_method, n_obs, num_samples, start, end, seed))
  write.csv(zdot_ci_df,
            sprintf("~/zdot_lorenz_increasing_snr_%s_N%s_B%s_snr_%s_%s_SEED_%s_ci_df.csv",
                    lasso_method, n_obs, num_samples, start, end, seed))
} else {
  write.csv(zdot_pe_df,
            sprintf("~/zdot_lorenz_increasing_snr_%s_%s_N%s_B%s_snr_%s_%s_SEED_%s_pe_df.csv",
                    lasso_method, alasso_weights_method, n_obs, num_samples, start, end, seed))
  write.csv(zdot_ci_df,
            sprintf("~/zdot_lorenz_increasing_snr_%s_%s_N%s_B%s_snr_%s_%s_SEED_%s_ci_df.csv",
                    lasso_method, alasso_weights_method, n_obs, num_samples, start, end, seed))
}
