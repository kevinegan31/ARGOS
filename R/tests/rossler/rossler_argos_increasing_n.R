...
num_state_var <- 3 # 3 state variables
init_sequence <- list()
for (i in 1:num_init) {
...
if (lasso_method == "lasso") {
  write.csv(
    xdot_pe_df,
    sprintf(
      "~/xdot_rossler_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    xdot_ci_normal_df,
    sprintf(
      "~/xdot_rossler_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    xdot_pe_df,
    sprintf(
      "~/xdot_rossler_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
  ...
  write.csv(
    xdot_ci_normal_df,
    sprintf(
      "~/xdot_rossler_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
...
if (lasso_method == "lasso") {
  write.csv(
    ydot_pe_df,
    sprintf(
      "~/ydot_rossler_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    ydot_ci_normal_df,
    sprintf(
      "~/ydot_rossler_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    ydot_pe_df,
    sprintf(
      "~/ydot_rossler_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
  ...
  write.csv(
    ydot_ci_normal_df,
    sprintf(
      "~/ydot_rossler_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
...
zdot_argos <- list()
for (i in seq_along(x_t_train)) {
  zdot_argos[[i]] <- boot_lasso(x_t = x_t_train[[i]],
                                monomial_degree = monomial_degree,
                                dt = dt,
                                alpha = alpha,
                                num_samples = num_samples,
...
zdot_argos_pe <- purrr::map(zdot_argos, `[[`, 1)
zdot_argos_ci_normal <- purrr::map(zdot_argos, `[[`, 2)
### We change the length of each element
### to the max length from the 'indx' (length<-)
### and thereby pad NA values at the end of the list elements
### with length less than the max length.
zdot_pe_df <- data.frame(lapply(zdot_argos_pe, "length<-",
                                max(lengths(zdot_argos_pe))))
colnames(zdot_pe_df) <- paste0("pe_", s)
### Normal CIs
zdot_ci_normal_max_length <-
  max(unlist(lapply(zdot_argos_ci_normal, length)))
zdot_ci_normal_df <- sapply(zdot_argos_ci_normal, function(x) {
  length(x) <- zdot_ci_normal_max_length
  return(x)
})
colnames(zdot_ci_normal_df) <- paste0("ci_", s)
zdot_argos_percent_zero_df <-
  data.frame(lapply(zdot_argos_percent_zero, "length<-",
                    max(lengths(
                      zdot_argos_percent_zero
                    ))))
colnames(zdot_argos_percent_zero_df) <- paste0("pz_", s)
### Write to CSV
if (lasso_method == "lasso") {
  write.csv(
    zdot_pe_df,
    sprintf(
      "~/zdot_rossler_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    zdot_ci_normal_df,
    sprintf(
      "~/zdot_rossler_increasing_n_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      start,
      end,
      num_samples,
  ...
  write.csv(
    zdot_pe_df,
    sprintf(
      "~/zdot_rossler_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_pe_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
  ...
  write.csv(
    zdot_ci_normal_df,
    sprintf(
      "~/zdot_rossler_increasing_n_%s_%s_N%s_%s_B%s_snr_%s_SEED_%s_ci_normal_df.csv",
      lasso_method,
      alasso_weights_method,
      start,
      end,
