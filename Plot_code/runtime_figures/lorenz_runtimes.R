...
zdot_lasso_grid_files <- mixedsort(str_subset(lasso_model_grid_pattern, "zdot"))
zdot_lasso_rt_list <- list()
for (i in seq_along(zdot_lasso_grid_files)) {
  zdot_lasso_rt_df <- load(zdot_lasso_grid_files[i])
  zdot_runtime_new <- sapply(zdot_run_time, FUN = function(x) {
    if (units(x) == "hours") {
      a <- as.numeric(x) * 60 * 60
    } else if (units(x) == "mins") {
      a <- as.numeric(x) * 60
  ...
  if (length(zdot_runtime_new) > num_init) {
    new_zdot_runtime <- zdot_runtime_new[c(1:num_init)]
  } else {
    new_zdot_runtime <- zdot_runtime_new
  }
  zdot_lasso_rt_list[[i]] <- new_zdot_runtime
}

xdot_lasso_rt_vector <- as.matrix(unlist(xdot_lasso_rt_list))
ydot_lasso_rt_vector <- as.matrix(unlist(ydot_lasso_rt_list))
zdot_lasso_rt_vector <- as.matrix(unlist(zdot_lasso_rt_list))
lasso_runtime_vector <-
  xdot_lasso_rt_vector + ydot_lasso_rt_vector + zdot_lasso_rt_vector
...
zdot_alasso_grid_files <- mixedsort(str_subset(alasso_model_grid_pattern, "zdot"))
zdot_alasso_rt_list <- list()
for (i in seq_along(zdot_alasso_grid_files)) {
  zdot_alasso_rt_df <- load(zdot_alasso_grid_files[i])
  zdot_runtime_new <- sapply(zdot_run_time, FUN = function(x) {
    if (units(x) == "hours") {
      a <- as.numeric(x) * 60 * 60
    } else if (units(x) == "mins") {
  ...
  zdot_alasso_rt_list[[i]] <- new_zdot_runtime
}

xdot_alasso_rt_vector <- as.matrix(unlist(xdot_alasso_rt_list))
ydot_alasso_rt_vector <- as.matrix(unlist(ydot_alasso_rt_list))
zdot_alasso_rt_vector <- as.matrix(unlist(zdot_alasso_rt_list))
alasso_runtime_vector <-
  xdot_alasso_rt_vector + ydot_alasso_rt_vector + zdot_alasso_rt_vector
