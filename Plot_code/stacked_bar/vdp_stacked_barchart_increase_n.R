...
library(stringr)
# setwd('D:/Users/Administrator/OneDrive - Durham University/Proc_Royal_Soc_A/Data/vdp/Bernoulli_Prediction_Models/Old/Old_reg_boot')
# setwd('/Users/kevinegan/OneDrive - Durham University/Proc_Royal_Soc_A/Data/vdp/Old_reg_boot')
...
ggplot_data_n <- function(algorithm, xdot_vdp, ydot_vdp, threshold=40){
  ...
  # xdot_vdp_reg_list <- split(xdot_vdp[-1], 1:31)
  ...
  # ydot_vdp_reg_list <- split(ydot_vdp[-1], 1:31)
  ...
  for(i in 1:nrow(plot_data3)){
    plot_data3$y_axis[i] <- 
      sum(dplyr::filter(plot_data2, eq==plot_data3$eq[i] & n==plot_data3$n[i])$value)
  }
...
## lasso --------------
xdot_vdp_lasso_reg_df <- read.csv("xdot_vdp_TAPER_lasso_increasing_n_prediction_models_new_sg.csv")[-1]
ydot_vdp_lasso_reg_df <- read.csv("ydot_vdp_TAPER_lasso_increasing_n_prediction_models_new_sg.csv")[-1]
...
## alasso ------------------
xdot_vdp_alasso_reg_df <- read.csv("xdot_vdp_TAPER_alasso_increasing_n_prediction_models_new_sg.csv")[-1]
ydot_vdp_alasso_reg_df <- read.csv("ydot_vdp_TAPER_alasso_increasing_n_prediction_models_new_sg.csv")[-1]
...
## STLS -------------------------
xdot_vdp_sindy_reg_df <- read.csv("xdot_vdp_pysindy_increasing_n_prediction_models_new_sg.csv")[-1]
# xdot_vdp_sindy_reg_df <- cbind(1, xdot_vdp_sindy_reg_df)
# colnames(xdot_vdp_sindy_reg_df) <- colnames(xdot_vdp_lasso_reg_df)
ydot_vdp_sindy_reg_df <- read.csv("ydot_vdp_pysindy_increasing_n_prediction_models_new_sg.csv")[-1]
# ydot_vdp_sindy_reg_df <- cbind(1, ydot_vdp_sindy_reg_df)
# colnames(ydot_vdp_sindy_reg_df) <- colnames(ydot_vdp_lasso_reg_df)
n_seq_desired <- c(1)
...
# source(paste0(
#   "D:/GitHub/",
#   "GitHub/Iterative-Sparse-Regression/Plot_Code/vdp/vdp_boxplot_number_of_selected_variables_increasing_n.R"))
## compose -----------------
library(gridExtra)
