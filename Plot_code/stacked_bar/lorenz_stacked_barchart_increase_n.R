...
library(stringr)
library(gridExtra)
# setwd('D:/Users/Administrator/OneDrive - Durham University/Proc_Royal_Soc_A/Data/Lorenz/Bernoulli_Prediction_Models/Old/Old_reg_boot')
# setwd('/Users/kevinegan/OneDrive - Durham University/Proc_Royal_Soc_A/Data/Lorenz/Old_reg_boot')
# xdot_lorenz_lasso <- read.csv("Bernoulli_Prediction_Models/xdot_lorenz_lasso_increasing_n_prediction_models_ordered.csv")[,-1]
file_wd <- paste(
...
# file_wd <- "D:/"
file_wd2 <- paste(file_wd, "GitHub/PrivateAutomaticSparseRegression/Data/Lorenz/Bernoulli_Prediction_Models/", sep = "")
setwd(file_wd2)
...
ggplot_data_n <- function(algorithm, xdot_lorenz, ydot_lorenz, zdot_lorenz,threshold=40){
  ...
  # xdot_lorenz_reg_list <- split(xdot_lorenz, 1:31)
  # apply(xdot_lorenz_reg_list[[1]], 2, function(c) sum(c!=0))
  x_dot_lorenz_terms_data = sapply(xdot_lorenz_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(x_dot_lorenz_terms_data)[1] <- "1"
  # n_seq[c(seq(1,30,5)[-6],22)]
  # index <- c(seq(1,30,5)[-6],22)
  plot_data_xdot <-
    cbind.data.frame(
      eq = 'xdot',
      names = rownames(x_dot_lorenz_terms_data),
      values = x_dot_lorenz_terms_data[, n_seq_desired]
    )
  
  ## y_dot
  ydot_lorenz_reg_list <- lapply(seq_along(n_seq), function(i){
    ydot_lorenz[((i-1)*100+1):((i-1)*100+1+99),]
  })
  # ydot_lorenz_reg_list <- split(ydot_lorenz, 1:31)
  # apply(ydot_lorenz_reg_list[[1]], 2, function(c) sum(c!=0))
  y_dot_lorenz_terms_data = sapply(ydot_lorenz_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(y_dot_lorenz_terms_data)[1] <- "1"
  # n_seq[c(seq(1,30,5)[-6],22)]
  # index <- c(seq(1,30,5)[-6],22)
  plot_data_ydot <-
    cbind.data.frame(
      eq = 'ydot',
      names = rownames(y_dot_lorenz_terms_data),
      values = y_dot_lorenz_terms_data[, n_seq_desired]
    )
  
  ## z_dot
  zdot_lorenz_reg_list <- lapply(seq_along(n_seq), function(i){
    zdot_lorenz[((i-1)*100+1):((i-1)*100+1+99),]
  })
  # zdot_lorenz_reg_list <- split(zdot_lorenz, 1:31)
  # apply(zdot_lorenz_reg_list[[1]], 2, function(c) sum(c!=0))
  z_dot_lorenz_terms_data = sapply(zdot_lorenz_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(z_dot_lorenz_terms_data)[1] <- "1"
  # n_seq[c(seq(1,30,5)[-6],22)]
  # index <- c(seq(1,30,5)[-6],22)
  plot_data_zdot <-
    cbind.data.frame(
      eq = 'zdot',
      names = rownames(z_dot_lorenz_terms_data),
      values = z_dot_lorenz_terms_data[, n_seq_desired]
    )
  ...
  plot_data1 <- rbind(dplyr::filter(plot_data1, value>threshold|names=='x'|names=='y') %>% dplyr::filter(eq=='xdot'),
                      dplyr::filter(plot_data1, value>threshold|names=='x'|names=='y'|names=='xz') %>% dplyr::filter(eq=='ydot'),
                      dplyr::filter(plot_data1, value>threshold|names=='z'|names=='xy') %>% dplyr::filter(eq=='zdot'))
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels =
             c(rownames(x_dot_lorenz_terms_data),'others'))
  
  # image(1 : length(colors0), 1, as.matrix(1 : length(colors0)), col = colors0 ,xlab =  "", ylab = "", xaxt = "n", yaxt = "n", bty = "n")
  # color_data <- cbind.data.frame(color=c(colors0,"#917127"), terms=c(rownames(x_dot_lorenz_terms_data),'others'))
  ...
  colors <- c(colors0[match(terms_show[-length(terms_show)],rownames(x_dot_lorenz_terms_data))], "#6c6c6c")
  # colors <- colors0[c(match(terms_show[-length(terms_show)],rownames(x_dot_lorenz_terms_data)),11)]
  # colors <- c(colors0[match(terms_show,rownames(x_dot_lorenz_terms_data))],"#917127")
  ...
  correct_terms <- c(which(plot_data2$eq=='xdot' & plot_data2$names=='x'),
                     ...
                     which(plot_data2$eq=='ydot' & plot_data2$names=='xz'),
                     which(plot_data2$eq=='zdot' & plot_data2$names=='z'),
                     which(plot_data2$eq=='zdot' & plot_data2$names=='xy'))
...
## lasso --------------
xdot_lorenz_lasso_reg_df <- read.csv("xdot_lorenz_lasso_increasing_n_prediction_models_new_sg.csv")[-1]
ydot_lorenz_lasso_reg_df <- read.csv("ydot_lorenz_lasso_increasing_n_prediction_models_new_sg.csv")[-1]
zdot_lorenz_lasso_reg_df <- read.csv("zdot_lorenz_lasso_increasing_n_prediction_models_new_sg.csv")[-1]
...
## alasso ------------------
xdot_lorenz_alasso_reg_df <- read.csv("xdot_lorenz_alasso_increasing_n_prediction_models_new_sg.csv")[-1]
ydot_lorenz_alasso_reg_df <- read.csv("ydot_lorenz_alasso_increasing_n_prediction_models_new_sg.csv")[-1]
zdot_lorenz_alasso_reg_df <- read.csv("zdot_lorenz_alasso_increasing_n_prediction_models_new_sg.csv")[-1]
...
## STLS -------------------------
xdot_lorenz_sindy_reg_df <- read.csv("xdot_lorenz_pysindy_increasing_n_prediction_models_new_sg.csv")[-1]
ydot_lorenz_sindy_reg_df <- read.csv("ydot_lorenz_pysindy_increasing_n_prediction_models_new_sg.csv")[-1]
zdot_lorenz_sindy_reg_df <- read.csv("zdot_lorenz_pysindy_increasing_n_prediction_models_new_sg.csv")[-1]
n_seq_desired <- c(1,6)
STLS_plot_n <- ggplot_data_n('SINDy with AIC', xdot_lorenz_sindy_reg_df, ydot_lorenz_sindy_reg_df, zdot_lorenz_sindy_reg_df,100)
...
# write.csv(x_dot_lorenz_lasso_terms_data,file = 'D:/Users/Administrator/OneDrive - Durham University/Proc_Royal_Soc_A/Plots/Lorenz/x_dot_lorenz_lasso_terms_data.csv')
...
STLS_plot_n <- ggplot_data_n('   ', xdot_lorenz_sindy_reg_df, ydot_lorenz_sindy_reg_df, zdot_lorenz_sindy_reg_df,70)
...
n_STLS <- arrangeGrob(STLS_plot2_n,STLS_plot2_n2,layout_matrix = layout_matrix,nrow=1,ncol=2)
## legend -------------------
# x_legend <- xdot_lorenz_lasso[,c(1:5,7,8,10)]
# x_legend <- xdot_lorenz_lasso[,1:11]
# x_legend[,-1] <- 4
...
# ggsave(ggplot_legend, filename='../../../../Figures/legned_3d.pdf', width = 4, height = 0.5)

## boxplot --------------------------
# source(paste0(
#   "D:/GitHub/",
#   "GitHub/Iterative-Sparse-Regression/Plot_Code/Lorenz/lorenz_boxplot_number_of_selected_variables_increasing_n.R"))
## compose -----------------
...
layout_matrix <- matrix(c(0,0,rep(1,20),rep(2,20),rep(3,20)), ncol=1)
stacked_lorenz_n <-
  grid.arrange(
