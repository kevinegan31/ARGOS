...
ggplot_data_n <- function(algorithm, xdot_cubic2d, ydot_cubic2d, threshold=40){
  ...
  plot_data_xdot <-
    cbind.data.frame(
      eq = 'xdot',
      names = rownames(x_dot_cubic2d_terms_data),
      values = x_dot_cubic2d_terms_data[, n_seq_desired]
    )
  
  ## y_dot
  ydot_cubic2d_reg_list <- lapply(seq_along(n_seq), function(i){
  ...
  plot_data_ydot <-
    cbind.data.frame(
      eq = 'ydot',
      names = rownames(y_dot_cubic2d_terms_data),
      values = y_dot_cubic2d_terms_data[, n_seq_desired]
    )
...
## lasso --------------
xdot_cubic2d_lasso_reg_df <- read.csv("N/cubic2d_inc_n_xdot_lasso_pred_models_new_sg.csv")[-1]
ydot_cubic2d_lasso_reg_df <- read.csv("N/cubic2d_inc_n_ydot_lasso_pred_models_new_sg.csv")[-1]
...
alasso_plot2_n <- alasso_plot_n+theme(legend.position='none')+
  annotate("rect", xmin = 5.5, xmax = 7.5, ymin = -10, ymax = 320,
           alpha = 0, color= "purple",lwd=1)
