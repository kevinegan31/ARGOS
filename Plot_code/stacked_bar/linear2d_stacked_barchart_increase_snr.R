...
ggplot_data_snr <- function(algorithm, xdot_linear2d, ydot_linear2d,threshold=40){
  ...
  ## y_dot
  ydot_linear2d_reg_list <- lapply(seq_along(snr_seq), function(i){
  ...
  plot_data_ydot <-
    cbind.data.frame(
      eq = 'ydot',
      names = rownames(y_dot_linear2d_terms_data),
      values = y_dot_linear2d_terms_data[, snr_seq_desired]
    )
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels =
             c(rownames(x_dot_linear2d_terms_data),'others'))
  ## need to replace x,y,z,... to x_1,x_2,x_3,...
  terms_show <- as.character(unique(sort(plot_data2$names)))
  plot_data2$names <-
    factor(plot_data2$names,
           levels = terms_show)
  colors <- c(colors0[match(terms_show[-length(terms_show)],rownames(x_dot_linear2d_terms_data))], "#6c6c6c")
...
## lasso --------------
xdot_linear2d_lasso <- read.csv("SNR/linear2d_inc_snr_xdot_lasso_pred_models_new_sg.csv")[,-1]
