...
ggplot_data_n <- function(algorithm, xdot_linear2d, ydot_linear2d,threshold=40){
  ...
  plot_data_xdot <-
    cbind.data.frame(
      eq = 'xdot',
      names = rownames(x_dot_linear2d_terms_data),
      values = x_dot_linear2d_terms_data[, n_seq_desired]
    )
  
  ## y_dot
  ydot_linear2d_reg_list <- lapply(seq_along(n_seq), function(i){
...
## lasso --------------
xdot_linear2d_lasso_reg_df <- read.csv("N/linear2d_inc_n_xdot_lasso_pred_models_new_sg.csv")[-1]
