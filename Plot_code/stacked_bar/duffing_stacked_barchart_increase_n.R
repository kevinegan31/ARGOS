...
ggplot_data_n <- function(algorithm, xdot_duffing, ydot_duffing,threshold=40){
  ...
  plot_data_xdot <- cbind.data.frame(eq = 'xdot', names = rownames(x_dot_duffing_terms_data),
      values = x_dot_duffing_terms_data[, n_seq_desired])
  
  ## y_dot
  ydot_duffing_reg_list <- lapply(seq_along(n_seq), function(i){
  ...
  plot_data_ydot <-
    cbind.data.frame(
      eq = 'ydot',
      names = rownames(y_dot_duffing_terms_data),
      values = y_dot_duffing_terms_data[, n_seq_desired]
    )
...
## lasso --------------
xdot_duffing_lasso_reg_df <- read.csv("N/duffing_inc_n_xdot_lasso_pred_models_new_sg.csv")[-1]
