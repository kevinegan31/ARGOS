...
ggplot_data_snr <- function(algorithm, xdot_duffing, ydot_duffing, threshold=40){
  ...
  x_dot_duffing_terms_data = sapply(xdot_duffing_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(x_dot_duffing_terms_data)[1] <- "1"
  plot_data_xdot <- cbind.data.frame(eq = 'xdot', names = rownames(x_dot_duffing_terms_data),
      values = x_dot_duffing_terms_data[, snr_seq_desired])
  
  ## y_dot
  ydot_duffing_reg_list <- lapply(seq_along(snr_seq), function(i){
    ydot_duffing[((i-1)*100+1):((i-1)*100+1+99),]
  })
  y_dot_duffing_terms_data = sapply(ydot_duffing_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(y_dot_duffing_terms_data)[1] <- "1"
  plot_data_ydot <- cbind.data.frame(eq = 'ydot', names = rownames(y_dot_duffing_terms_data),
      values = y_dot_duffing_terms_data[, snr_seq_desired])
  ...
  plot_data1 <-
    rbind(
      dplyr::filter(plot_data1, value > threshold | names == 'y') %>% dplyr::filter(eq == 'xdot'),
      dplyr::filter(plot_data1, value > threshold |
               names == 'x' | names == 'y' | names == 'xxx') %>% dplyr::filter(eq == 'ydot')
    )
  ## incorrect terms, frequency <= threshold, and sum them
  others_values_xdot <-
    apply(subset(plot_data, plot_data[, 1] == 'xdot')[, -c(1, 2)], 2, function(x) {
      terms <- subset(plot_data, plot_data[, 1] == 'xdot')[, 2]
      index <- which(x < threshold & terms != 'y')
      sum(x[index])
    })
  others_values_ydot <-
    apply(subset(plot_data, plot_data[, 1] == 'ydot')[, -c(1, 2)], 2, function(x) {
      terms <- subset(plot_data, plot_data[, 1] == 'ydot')[, 2]
      index <- which(x < threshold & terms != 'x' &
                       terms != 'y' & terms != 'xxx')
      sum(x[index])
    })
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels = terms_show)
  colors <- c(colors0[match(terms_show[-length(terms_show)],rownames(x_dot_duffing_terms_data))], "#6c6c6c")
...
## lasso --------------
xdot_duffing_lasso <- read.csv("SNR/duffing_inc_snr_xdot_lasso_pred_models_new_sg.csv")[,-1]
ydot_duffing_lasso <- read.csv("SNR/duffing_inc_snr_ydot_lasso_pred_models_new_sg.csv")[,-1]
lasso_plot_snr <- ggplot_data_snr('ARGOS-Lasso', xdot_duffing_lasso, ydot_duffing_lasso,25)
lasso_plot2_snr <- lasso_plot_snr+theme(legend.position='none')
## alasso ------------------
xdot_duffing_alasso <- read.csv("SNR/duffing_inc_snr_xdot_alasso_pred_models_new_sg.csv")[,-1]
ydot_duffing_alasso <- read.csv("SNR/duffing_inc_snr_xdot_alasso_pred_models_new_sg.csv")[,-1]
alasso_plot_snr <- ggplot_data_snr('ARGOS-Adaptive Lasso', xdot_duffing_alasso, ydot_duffing_alasso,25)
