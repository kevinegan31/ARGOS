...
library(stringr)
# setwd('D:/Users/Administrator/OneDrive - Durham University/Proc_Royal_Soc_A/Data/rossler')
# setwd('/Users/kevinegan/OneDrive - Durham University/Proc_Royal_Soc_A/Data/rossler')
# xdot_rossler_lasso <- read.csv("Bernoulli_Prediction_Models/xdot_rossler_lasso_increasing_n_prediction_models_ordered.csv")[,-1]
file_wd <- paste(
...
# file_wd <- "D:/"
file_wd2 <- paste(file_wd, "GitHub/PrivateAutomaticSparseRegression/Data/rossler/Bernoulli_Prediction_Models/", sep = "")
setwd(file_wd2)
...
ggplot_data_snr <- function(algorithm, xdot_rossler, ydot_rossler, zdot_rossler, threshold = 40){
  # xdot_rossler = xdot_rossler_lasso;ydot_rossler = ydot_rossler_lasso;zdot_rossler = zdot_rossler_lasso;algorithm='lasso'
  ## x_dot
  xdot_rossler_reg_list <- lapply(seq_along(snr_seq), function(i){
    xdot_rossler[((i-1)*100+1):((i-1)*100+1+99),]
  })
  # xdot_rossler_reg_list <- split(xdot_rossler, seq_along(snr_seq))
  # apply(xdot_rossler_reg_list[[1]], 2, function(c) sum(c!=0))
  x_dot_rossler_terms_data = sapply(xdot_rossler_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(x_dot_rossler_terms_data)[1] <- "1"
  # snr_seq[c(seq(1,30,5)[-6],22)]
  # index <- c(seq(1,30,5)[-6],22)
  plot_data_xdot <-
    cbind.data.frame(
      eq = 'xdot',
      names = rownames(x_dot_rossler_terms_data),
      values = x_dot_rossler_terms_data[, snr_seq_desired]
    )
  
  ## y_dot
  ydot_rossler_reg_list <- lapply(seq_along(snr_seq), function(i){
    ydot_rossler[((i-1)*100+1):((i-1)*100+1+99),]
  })
  # ydot_rossler_reg_list <- split(ydot_rossler, seq_along(snr_seq))
  # apply(ydot_rossler_reg_list[[1]], 2, function(c) sum(c!=0))
  y_dot_rossler_terms_data = sapply(ydot_rossler_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(y_dot_rossler_terms_data)[1] <- "1"
  # snr_seq[c(seq(1,30,5)[-6],22)]
  # index <- c(seq(1,30,5)[-6],22)
  plot_data_ydot <-
    cbind.data.frame(
      eq = 'ydot',
      names = rownames(y_dot_rossler_terms_data),
      values = y_dot_rossler_terms_data[, snr_seq_desired]
    )
  
  ## z_dot
  zdot_rossler_reg_list <- lapply(seq_along(snr_seq), function(i){
    zdot_rossler[((i-1)*100+1):((i-1)*100+1+99),]
  })
  # zdot_rossler_reg_list <- split(zdot_rossler, seq_along(snr_seq))
  # apply(zdot_rossler_reg_list[[1]], 2, function(c) sum(c!=0))
  z_dot_rossler_terms_data = sapply(zdot_rossler_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(z_dot_rossler_terms_data)[1] <- "1"
  # snr_seq[c(seq(1,30,5)[-6],22)]
  # index <- c(seq(1,30,5)[-6],22)
  plot_data_zdot <-
    cbind.data.frame(
      eq = 'zdot',
      names = rownames(z_dot_rossler_terms_data),
      values = z_dot_rossler_terms_data[, snr_seq_desired]
    )
  ...
  plot_data1 <- rbind(dplyr::filter(plot_data1, value>threshold|names=='y'|names=='z') %>% dplyr::filter(eq=='xdot'),
                      dplyr::filter(plot_data1, value>threshold|names=='x'|names=='y') %>% dplyr::filter(eq=='ydot'),
                      dplyr::filter(plot_data1, value>threshold|names == '1' | names=='z' | names == 'xz') %>% dplyr::filter(eq=='zdot'))
  ## incorrect terms, frequency <= 20, and sum them
  others_values_xdot <- apply(subset(plot_data,plot_data[,1]=='xdot')[,-c(1,2)], 2, function(x){
    terms <- subset(plot_data,plot_data[,1]=='xdot')[,2]
    index <- which(x<threshold & terms!='y'& terms!='z')
    sum(x[index])
  })
  ...
  others_values_zdot <- apply(subset(plot_data,plot_data[,1]=='zdot')[,-c(1,2)], 2, function(x){
    ...
    index <- which(x<threshold & terms!='1'& terms!='z'& terms!='xz')
    sum(x[index])
  })
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels =
             c(rownames(x_dot_rossler_terms_data),'others'))
  
  # image(1 : length(colors0), 1, as.matrix(1 : length(colors0)), col = colors0 ,xlab =  "", ylab = "", xaxt = "n", yaxt = "n", bty = "n")
  # color_data <- cbind.data.frame(color=c(colors0,"#917127"), terms=c(rownames(x_dot_rossler_terms_data),'others'))
  ...
  colors <- c(colors0[match(terms_show[-length(terms_show)],rownames(x_dot_rossler_terms_data))], "#6c6c6c")
  # colors <- colors0[c(match(terms_show[-length(terms_show)],rownames(x_dot_rossler_terms_data)),11)]
  # colors <- c(colors0[match(terms_show,rownames(x_dot_rossler_terms_data))],"#917127")
  ...
  correct_terms <- c(which(plot_data2$eq=='xdot' & plot_data2$names=='y'),
                     which(plot_data2$eq=='xdot' & plot_data2$names=='z'),
                     which(plot_data2$eq=='ydot' & plot_data2$names=='x'),
                     which(plot_data2$eq=='ydot' & plot_data2$names=='y'),
                     which(plot_data2$eq=='zdot' & plot_data2$names=='1'),
                     which(plot_data2$eq=='zdot' & plot_data2$names=='z'),
                     which(plot_data2$eq=='zdot' & plot_data2$names=='xz'))
  ...
  try(plot_data4[which(plot_data4$value<threshold),]$label2 <- NA, TRUE)
...
## lasso --------------
xdot_rossler_lasso <- read.csv("xdot_rossler_lasso_increasing_snr_prediction_models_N_5000_TAPER_new_sg.csv")[,-1]
ydot_rossler_lasso <- read.csv("ydot_rossler_lasso_increasing_snr_prediction_models_N_5000_TAPER_new_sg.csv")[,-1]
zdot_rossler_lasso <- read.csv("zdot_rossler_lasso_increasing_snr_prediction_models_N_5000_TAPER_new_sg.csv")[,-1]
...
## alasso ------------------
xdot_rossler_alasso <- read.csv("xdot_rossler_alasso_increasing_snr_prediction_models_N_5000_TAPER_new_sg.csv")[,-1]
ydot_rossler_alasso <- read.csv("ydot_rossler_alasso_increasing_snr_prediction_models_N_5000_TAPER_new_sg.csv")[,-1]
zdot_rossler_alasso <- read.csv("zdot_rossler_alasso_increasing_snr_prediction_models_N_5000_TAPER_new_sg.csv")[,-1]
...
## STLS -------------------------
xdot_rossler_STLS <- read.csv("xdot_rossler_sindy_increasing_snr_prediction_models_new_sg.csv")[,-1]
ydot_rossler_STLS <- read.csv("ydot_rossler_sindy_increasing_snr_prediction_models_new_sg.csv")[,-1]
zdot_rossler_STLS <- read.csv("Zdot_rossler_sindy_increasing_snr_prediction_models_new_sg.csv")[,-1]
STLS_plot_snr <- ggplot_data_snr('SINDy with AIC', xdot_rossler_STLS, ydot_rossler_STLS, zdot_rossler_STLS,60)
STLS_plot2_snr <- STLS_plot_snr+theme(legend.position='none')
# legend <- get_legend(STLS_plot+theme(legend.position='bottom'))
# cowplot::plot_grid(legend)

# write.csv(x_dot_rossler_lasso_terms_data,file = 'D:/Users/Administrator/OneDrive - Durham University/Proc_Royal_Soc_A/Plots/rossler/x_dot_rossler_lasso_terms_data.csv')
...
## legend -------------------
# x_legend <- xdot_rossler_lasso[,c(1:5,7,8,10)]
# x_legend <- xdot_rossler_lasso[,1:11]
# x_legend[,-1] <- 4
...
# source(paste0(
#   "D:/GitHub/",
#   "Iterative-Sparse-Regression/Plot_Code/rossler/rossler_boxplot_number_of_selected_variables_increasing_snr.R"))
## compose -----------------
library(gridExtra)
...
ggsave(stacked_rossler, filename = '../../../Figures/stacked_rossler_snr.pdf', width = 10, height = 15, dpi = 300)
ggsave(stacked_rossler, filename = '../../../Figures/stacked_rossler_snr.png', width = 10, height = 15, dpi = 300)
