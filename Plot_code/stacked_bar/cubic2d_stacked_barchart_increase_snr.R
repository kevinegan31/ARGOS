...
ggplot_data_snr <- function(algorithm, xdot_cubic2d, ydot_cubic2d, threshold=40){
  # xdot_cubic2d = xdot_cubic2d_alasso;ydot_cubic2d = ydot_cubic2d_alasso;algorithm='alasso'
  ## x_dot
  xdot_cubic2d_reg_list <- lapply(seq_along(snr_seq), function(i){
    xdot_cubic2d[((i-1)*100+1):((i-1)*100+1+99),]
  })
  x_dot_cubic2d_terms_data = sapply(xdot_cubic2d_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(x_dot_cubic2d_terms_data)[1] <- "1"
  plot_data_xdot <-
    cbind.data.frame(eq = 'xdot', names = rownames(x_dot_cubic2d_terms_data),
      values = x_dot_cubic2d_terms_data[, snr_seq_desired])
  
  ## y_dot
  ydot_cubic2d_reg_list <- lapply(seq_along(snr_seq), function(i){
    ydot_cubic2d[((i-1)*100+1):((i-1)*100+1+99),]
  })
  y_dot_cubic2d_terms_data = sapply(ydot_cubic2d_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(y_dot_cubic2d_terms_data)[1] <- "1"
  plot_data_ydot <-
    cbind.data.frame(eq = 'ydot', names = rownames(y_dot_cubic2d_terms_data),
      values = y_dot_cubic2d_terms_data[, snr_seq_desired])
  ...
  plot_data1 <- rbind(dplyr::filter(plot_data1, value>threshold|names=='xxx'|names=='yyy') %>% dplyr::filter(eq=='xdot'),
                      dplyr::filter(plot_data1, value>threshold|names=='xxx'|names=='yyy') %>% dplyr::filter(eq=='ydot'))
  ...
  others_values_xdot <- apply(subset(plot_data,plot_data[,1]=='xdot')[,-c(1,2)], 2, function(x){
    ...
    index <- which(x<threshold & terms!='xxx'& terms!='yyy')
    sum(x[index])
  })
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels =
             c(rownames(x_dot_cubic2d_terms_data),'others'))
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels = terms_show)
  colors <- c(colors0[match(terms_show[-length(terms_show)],rownames(x_dot_cubic2d_terms_data))], "#6c6c6c")
  ...
  correct_terms <- c(which(plot_data2$eq=='xdot' & plot_data2$names=='xxx'),
                     which(plot_data2$eq=='xdot' & plot_data2$names=='yyy'),
                     which(plot_data2$eq=='ydot' & plot_data2$names=='xxx'),
                     which(plot_data2$eq=='ydot' & plot_data2$names=='yyy'))
...
## lasso --------------
xdot_cubic2d_lasso <- read.csv("SNR/cubic2d_inc_snr_xdot_lasso_pred_models_new_sg.csv")[,-1]
ydot_cubic2d_lasso <- read.csv("SNR/cubic2d_inc_snr_ydot_lasso_pred_models_new_sg.csv")[,-1]
lasso_plot_snr <- ggplot_data_snr('ARGOS-Lasso', xdot_cubic2d_lasso, ydot_cubic2d_lasso,27)
lasso_plot2_snr <- lasso_plot_snr+theme(legend.position='none')

## alasso ------------------
xdot_cubic2d_alasso <- read.csv("SNR/cubic2d_inc_snr_xdot_alasso_pred_models_new_sg.csv")[,-1]
ydot_cubic2d_alasso <- read.csv("SNR/cubic2d_inc_snr_ydot_alasso_pred_models_new_sg.csv")[,-1]
alasso_plot_snr <- ggplot_data_snr('ARGOS-Adaptive Lasso', xdot_cubic2d_alasso, ydot_cubic2d_alasso,27)
alasso_plot2_snr <- alasso_plot_snr+theme(legend.position='none')

## STLS -------------------------
xdot_cubic2d_STLS <- read.csv("SNR/cubic2d_inc_snr_xdot_sindy_pred_models_new_sg.csv")[,-1]
ydot_cubic2d_STLS <- read.csv("SNR/cubic2d_inc_snr_ydot_sindy_pred_models_new_sg.csv")[,-1]
STLS_plot_snr <- ggplot_data_snr('SINDy with AIC', xdot_cubic2d_STLS, ydot_cubic2d_STLS)
...
file_wd <- paste(
  "/Users/kevinegan/Documents/GitHub.nosync/PrivateAutomaticSparseRegression/Figures/",
  # "~//GitHub/ARGOS/",
  sep = ""
)
setwd(file_wd)
ggsave(stacked_cubic2d_snr, filename = './stacked_cubic2d_snr.pdf', width = 10, height = 15, dpi = 300)
ggsave(stacked_cubic2d_snr, filename = './stacked_cubic2d_snr.png', width = 10, height = 15, dpi = 300)
