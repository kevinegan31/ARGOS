...
ggplot_data_n <- function(algorithm, xdot_LV, ydot_LV, threshold=40){
  ...
  x_dot_LV_terms_data = sapply(xdot_LV_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(x_dot_LV_terms_data)[1] <- "1"
  plot_data_xdot <- cbind.data.frame(eq = 'xdot',names = rownames(x_dot_LV_terms_data),
      values = x_dot_LV_terms_data[, n_seq_desired])
  
  ## y_dot
  ydot_LV_reg_list <- lapply(seq_along(n_seq), function(i){
    ydot_LV[((i-1)*100+1):((i-1)*100+1+99),]
  })
  y_dot_LV_terms_data = sapply(ydot_LV_reg_list, function(x) apply(x,2,function(c) sum(c!=0)))
  rownames(y_dot_LV_terms_data)[1] <- "1"
  plot_data_ydot <- cbind.data.frame(eq = 'ydot',names = rownames(y_dot_LV_terms_data),
      values = y_dot_LV_terms_data[, n_seq_desired])
  ...
  plot_data1 <- rbind(dplyr::filter(plot_data1, value>threshold|names=='x'|names=='xy') %>% dplyr::filter(eq=='xdot'),
                      dplyr::filter(plot_data1, value>threshold|names=='y'|names=='xy') %>% dplyr::filter(eq=='ydot'))
  ...
  others_values_xdot <- apply(as.data.frame(subset(plot_data,plot_data[,1]=='xdot')[,-c(1,2)]), 2, function(x){
    ...
    index <- which(x<=threshold & terms!='x' & terms!='xy')
    sum(x[index])
  })
  others_values_ydot <- apply(as.data.frame(subset(plot_data,plot_data[,1]=='ydot')[,-c(1,2)]), 2, function(x){
    ...
    index <- which(x<=threshold & terms!='y' & terms!='xy')
    sum(x[index])
  })
  ...
  plot_data2$names <-
    factor(plot_data2$names,
           levels = terms_show)
  colors <- c(colors0[match(terms_show[-length(terms_show)],rownames(x_dot_LV_terms_data))], "#6c6c6c")
  ...
  correct_terms <- c(which(plot_data2$eq=='xdot' & plot_data2$names=='x'),
                     which(plot_data2$eq=='xdot' & plot_data2$names=='xy'),
                     which(plot_data2$eq=='ydot' & plot_data2$names=='y'),
                     which(plot_data2$eq=='ydot' & plot_data2$names=='xy'))
...
## lasso --------------
xdot_LV_lasso_reg_df <- read.csv("N/LV_inc_n_xdot_lasso_pred_models_new_sg.csv")[-1]
ydot_LV_lasso_reg_df <- read.csv("N/LV_inc_n_ydot_lasso_pred_models_new_sg.csv")[-1]
lasso_plot_n <- ggplot_data_n('ARGOS-Lasso', xdot_LV_lasso_reg_df, ydot_LV_lasso_reg_df)
...
## alasso ------------------
xdot_LV_alasso_reg_df <- read.csv("N/LV_inc_n_xdot_alasso_pred_models_new_sg.csv")[-1]
ydot_LV_alasso_reg_df <- read.csv("N/LV_inc_n_ydot_alasso_pred_models_new_sg.csv")[-1]
alasso_plot_n <- ggplot_data_n('ARGOS-Adaptive Lasso', xdot_LV_alasso_reg_df, ydot_LV_alasso_reg_df)
alasso_plot2_n <- alasso_plot_n+theme(legend.position='none')
alasso_plot2_n <- alasso_plot_n+theme(legend.position='none')+
  annotate("rect", xmin = 3.5, xmax = 7.5, ymin = -10, ymax = 320,
           alpha = 0, color= "purple",lwd=1)
## STLS -------------------------
xdot_LV_sindy_reg_df <- read.csv("N/LV_inc_n_xdot_sindy_pred_models_new_sg.csv")[-1]
ydot_LV_sindy_reg_df <- read.csv("N/LV_inc_n_ydot_sindy_pred_models_new_sg.csv")[-1]
n_seq_desired = 1
STLS_plot_n <- ggplot_data_n('SINDy with AIC', xdot_LV_sindy_reg_df, ydot_LV_sindy_reg_df,95)
