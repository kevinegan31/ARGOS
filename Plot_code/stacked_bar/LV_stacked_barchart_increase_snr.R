...
ggplot_data_snr <- function(algorithm, xdot_LV, ydot_LV, threshold = 40){
  ## x_dot
  xdot_LV_reg_list <- lapply(seq_along(snr_seq), function(i){
  ...
  plot_data_xdot <- cbind.data.frame(eq = 'xdot', names = rownames(x_dot_LV_terms_data),
      values = x_dot_LV_terms_data[, snr_seq_desired])
  
  ## y_dot
  ydot_LV_reg_list <- lapply(seq_along(snr_seq), function(i){
  ...
  plot_data_ydot <- cbind.data.frame(eq = 'ydot', names = rownames(y_dot_LV_terms_data),
      values = y_dot_LV_terms_data[, snr_seq_desired])
  ...
  plot_data1 <- rbind(dplyr::filter(plot_data1, value>threshold|names=='x'|names=='xy') %>% dplyr::filter(eq=='xdot'),
                      dplyr::filter(plot_data1, value>threshold|names=='xy'|names=='y') %>% dplyr::filter(eq=='ydot'))
  ...
  others_values_ydot <- apply(subset(plot_data,plot_data[,1]=='ydot')[,-c(1,2)], 2, function(x){
    ...
    index <- which(x<=threshold & terms!='xy'& terms!='y')
    sum(x[index])
  })
...
## lasso --------------
xdot_LV_lasso <- read.csv("SNR/LV_inc_snr_xdot_lasso_pred_models_new_sg.csv")[,-1]
...
ggsave(stacked_LV_snr, filename = './stacked_LV_snr.pdf', width = 10, height = 15, dpi = 300)
ggsave(stacked_LV_snr, filename = './stacked_LV_snr.png', width = 10, height = 15, dpi = 300)
