...
x_orig_init_conditions = np.random.uniform(-2,2,num_validation_sets,)
y_orig_init_conditions = np.random.uniform(-6,6,num_validation_sets,)
original_init_condition_vector = [i for tup in zip(x_orig_init_conditions,y_orig_init_conditions) for i in tup]
original_init_condition_matrix = np.split(np.array(original_init_condition_vector), 100)
x_val_init_conditions = np.random.uniform(-2,2,num_validation_sets,)
y_val_init_conditions = np.random.uniform(-6,6,num_validation_sets,)
validation_init_condition_vector = [i for tup in zip(x_val_init_conditions,y_val_init_conditions) for i in tup]
...
def get_results(i):
    stls_model = []
    validation_data = validation_data_generate(validation_init_condition_matrix, t_total_list[i], f, dt, snr_volt, true_matrix_a)
    for j in range(len(orig_init_condition_vector_matrix)):
        training_data = training_data_generate(orig_init_condition_vector_matrix[j], t_total_list[i], f, dt, snr_volt, true_matrix_a)
...
xdot_models.to_csv('./sindy_aic_duffing_snr_%s_increasing_N%s_%s_max_iter_%s_lambda_10%s_%s_num_lambda_%s_xdot.csv' % (snr, START, END, max_iter_num, lambda_min, lambda_max, num_lambda))
ydot_models.to_csv('./sindy_aic_duffing_snr_%s_increasing_N%s_%s_max_iter_%s_lambda_10%s_%s_num_lambda_%s_ydot.csv' % (snr, START, END, max_iter_num, lambda_min, lambda_max, num_lambda))
