...
def get_results(i):
    stls_model = []
    validation_data = validation_data_generate(validation_init_condition_matrix, t_total_list[i], f, dt, snr_volt, mu)
    for j in range(len(orig_init_condition_vector_matrix)):
        training_data = training_data_generate(orig_init_condition_vector_matrix[j], t_total_list[i], f, dt, snr_volt, mu)
...
xdot_models.to_csv('./sindy_aic_vdp_snr_%s_increasing_N%s_%s_max_iter_%s_lambda_10%s_%s_num_lambda_%s_xdot.csv' % (snr, START, END, max_iter_num, lambda_min, lambda_max, num_lambda))
ydot_models.to_csv('./sindy_aic_vdp_snr_%s_increasing_N%s_%s_max_iter_%s_lambda_10%s_%s_num_lambda_%s_ydot.csv' % (snr, START, END, max_iter_num, lambda_min, lambda_max, num_lambda))
