...
n=int(len(f(dummy_init_conditions,0,mu)))
orig_init_condition_vector = (np.random.uniform(-4,4, n*100,))
...
validation_init_condition_vector = (np.random.uniform(-4,4, n*100,))
...
def get_results(i):
    stls_model = []
    validation_data = validation_data_generate(validation_init_condition_matrix, t_total, f, dt, snr_volt[i], mu)
    for j in range(len(orig_init_condition_vector_matrix)):
        training_data = training_data_generate(orig_init_condition_vector_matrix_new[j], t_total, f, dt, snr_volt[i], mu)
...
xdot_models.to_csv('./sindy_aic_vdp_increasing_snr_%s_%s_N%s_max_iter_%s_SEED_%s_lambda_%s_%s_num_lambda_%s_xdot.csv' % (noise_start, noise_end, n_obs, max_iter_num, seed, lambda_min, lambda_max, num_lambda))
ydot_models.to_csv('./sindy_aic_vdp_increasing_snr_%s_%s_N%s_max_iter_%s_SEED_%s_lambda_%s_%s_num_lambda_%s_ydot.csv' % (noise_start, noise_end, n_obs, max_iter_num, seed, lambda_min, lambda_max, num_lambda))
