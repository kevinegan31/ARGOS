...
def f(x, t, a):
    return [a[0] * x[0] + a[1] * x[1], a[2] * x[0] + a[3] * x[1], a[4] * x[2]]
...
validation_init_condition_vector = 10 ** (
    np.random.uniform(
        -1,
        3,
        n * 100,
...
validation_init_condition_matrix_new = validation_init_condition_matrix * int(
    len(noise_levels)
)
...
def get_results(i):
    ...
    validation_data = validation_data_generate(
        validation_init_condition_matrix, t_total, f, dt, snr_volt[i], true_matrix_a
    )
    for j in range(len(orig_init_condition_vector_matrix)):
        training_data = training_data_generate(
            orig_init_condition_vector_matrix_new[j],
            t_total,
            f,
            dt,
            snr_volt[i],
...
for i in range(0, len(stls_models_new)):
    ...
    zdot_models[i,] = stls_models_new[
        i
    ][:, 2]


xdot_models.to_csv(
    "./sindy_aic_linear3d_increasing_snr_%s_%s_N%s_max_iter_%s_SEED_%s_lambda_%s_%s_num_lambda_%s_xdot.csv"
    % (
        noise_start,
        noise_end,
        n_obs,
...
ydot_models.to_csv(
    "./sindy_aic_linear3d_increasing_snr_%s_%s_N%s_max_iter_%s_SEED_%s_lambda_%s_%s_num_lambda_%s_ydot.csv"
    % (
        noise_start,
        noise_end,
        n_obs,
...
zdot_models.to_csv(
    "./sindy_aic_linear3d_increasing_snr_%s_%s_N%s_max_iter_%s_SEED_%s_lambda_%s_%s_num_lambda_%s_zdot.csv"
    % (
        noise_start,
        noise_end,
        n_obs,
